...
from c_infrastructure.technical.yaml_technical_settings_repository import YamlTechnicalSettingsRepository
...
def get_chroma_repository(
    config: Annotated[AppConfig, Depends(get_settings)], logger: Annotated[ILoggingProvider, Depends(get_logger)]
) -> ChromaRepositoryAdapter:
    return ChromaRepositoryAdapter(config=config, logger=logger)


def get_conversation_repository(
    repository: Annotated[ChromaRepositoryAdapter, Depends(get_chroma_repository)],
) -> IConversationRepository:
    return repository


def get_knowledge_repository(repository: Annotated[ChromaRepositoryAdapter, Depends(get_chroma_repository)]) -> IKnowledgeRepository:
    return repository


def get_signal_repository(
    database: Annotated[DatabaseConnector, Depends(get_db_connector)], logger: Annotated[ILoggingProvider, Depends(get_logger)]
) -> ISignalRepository:
    return SignalRepository(db=database, logger=logger)


def get_watchlist_repository(
    database: Annotated[DatabaseConnector, Depends(get_db_connector)],
    logger: Annotated[ILoggingProvider, Depends(get_logger)],
    market_clock: Annotated[IMarketClock, Depends(get_market_clock)],
) -> IWatchlistRepository:
    return WatchlistRepository(db=database, logger=logger, market_clock=market_clock)


def get_technical_settings_repository(config: Annotated[AppConfig, Depends(get_settings)]) -> ITechnicalSettingsRepository:
    settings_path = config.project_root / "config" / "technical.yaml"
    return YamlTechnicalSettingsRepository(settings_path)
...
def get_execution_provider(
    ...
    return MockExecutionProvider(db=database, config=config, logger=logger)
