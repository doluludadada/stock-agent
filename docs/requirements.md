...
---
## US-006 — Short-Term Social Buzz Opportunity Workflow

**Priority:** P0

As a user,
I want the system to analyse recently discussed stocks as short-term opportunities,
so that unusual social momentum can still be discovered even when the stock would fail the normal Full Cycle strategy.

### Acceptance Criteria

- `BuzzScanner` obtains recently discussed stocks.
- Buzz uses `StrategyName.BUZZ`; it must not reuse the active Full Cycle strategy.
- Historical OHLCV is loaded and technical indicators are calculated.
- Buzz hard failures are limited to basic tradeability and safety:
  - minimum price
  - minimum liquidity
  - maximum daily range
- RSI, volume expansion, MACD, and Stochastic are confirmation signals:
  - failure reduces technical quality
  - failure does not automatically eliminate the stock
- Stocks passing Buzz hard requirements are marked and persisted as `WatchlistType.BUZZ`.
- News and social articles are analysed using the momentum AI prompt.
- Technical score and composite score are calculated.
- Trading signals are generated and persisted.
- `OrderExecution` receives generated executable signals.
- Existing order validation remains responsible for market-open, quantity, price, cash, and broker constraints.
- The workflow returns a complete `PipelineStatus`.

### Explicit Non-Goals

- Social mention/push thresholds remain US-007.
- Automatic order-mode policy remains US-009.
- Realtime data freshness validation remains US-201.
- Do not append 5-minute realtime bars into daily OHLCV history to calculate daily indicators.
---

## US-007 — Apply Social Buzz Thresholds

**Priority: P1**

As a user,

I want the system to filter social discussions by measurable attention before running expensive stock analysis,

so that low-interest and noisy discussions do not enter the Buzz workflow.

### Business Rules

- Buzz represents **attention**, not bullish or bearish sentiment.
    
- A highly discussed bearish stock is still a Buzz stock.
    
- Social providers may expose different engagement mechanisms.
    
- Domain/application logic must not depend directly on PTT-specific concepts such as `推`.
    
- Sentiment analysis is not part of US-007.
    

### Acceptance Criteria

- Social discussions are grouped by `stock_id`.
    
- The system calculates a `mention_count` for each stock.
    
- The system calculates an engagement metric for each stock.
    
- Configured Buzz thresholds are applied.
    
- Existing `SocialBuzzCriteria.is_trending()` is used to determine eligibility.
    
- Stocks that do not satisfy either Buzz threshold do not enter the US-006 analysis workflow.
    
- All qualifying articles remain attached to the corresponding stock.
    
- Statistics record:
    
    - stocks before Buzz filtering
        
    - stocks after Buzz filtering
        
- Existing US-006 technical, AI, watchlist, signal, and order behavior is unchanged.
...
- engagement_score  
    ...
    └── PASS → buzz_stocks  
    ↓  
    US-006 Buzz Workflow
    

### Explicit Non-Goals

US-007 does not implement:
...
- PTT daily-chat sentiment analysis
    

Those belong to a later Social Sentiment story.
...
- 下次分析時可以取得相關歷史內容。
--- 
## US-011 — Social Sentiment Analysis

**Priority: P2**

As a user,

I want the system to understand whether social discussion around a trending stock is bullish, bearish, neutral, or unclear,

so that I can distinguish social attention from actual market opinion.

### Business Rules

- Buzz and sentiment are independent concepts.
    
- A stock can have high Buzz while sentiment is strongly bearish.
    
- Sentiment analysis runs only after cheap Buzz filtering.
    
- The application must not depend on a specific AI vendor or model.
    
- Sentiment inference should run centrally on the backend rather than on the user's phone.
    
- Results may be cached because the same public social discussion is shared by many users.
    
- The initial implementation should reuse an existing pretrained classifier or model rather than training a language model from scratch.
...
- Sentiment analysis processes social content belonging to a qualified trending stock.
    
- Results include sentiment label and confidence.
    
- Multiple comments can be aggregated into stock-level sentiment.
    
- The system preserves disagreement instead of treating equal bullish/bearish opinions as neutral.
    
- Stock-level output can represent:
...
- Model implementation is replaceable behind an application/domain-facing interface.
...
- Client devices do not need to execute the model.
    
- Sentiment results can be cached and later persisted.
    
- Sentiment must not automatically generate BUY or SELL decisions.
    

### Future Training Path

Existing pretrained classifier  
↓  
Collect PTT / Reddit dataset  
↓  
Manually label a small evaluation dataset  
↓  
Benchmark baseline  
↓  
Fine-tune only if necessary  
↓  
Deploy small backend classifier

### Explicit Non-Goals

US-011 does not require:

- training an LLM from scratch
    
- running an LLM on mobile devices
    
- using sentiment directly as an automatic trade trigger
    
- cross-platform sentiment weighting
...
## US-304 — Configurable Technical Strategy Composition

**Priority:** P3

As an advanced user,  
I want technical strategy rule composition to be configurable,  
so that Full Cycle, Buzz, and future strategies can change their screening strictness from the UI without requiring Python code changes.

### Acceptance Criteria

- Strategy configuration can define which criteria belong to:
...
- `TechnicalStrategyFactory` no longer contains hard-coded methods such as `create_buzz()`, `create_moderate()`, and strategy-specific rule-list methods.
    
- The factory only converts validated strategy configuration into typed `TechnicalCriterion` objects.
    
- Unknown criterion names are rejected during configuration loading.
    
- Invalid criterion parameters are rejected before a workflow starts.
    
- UI configuration never provides Python class names or dynamic imports directly.
    
- Domain criteria remain normal typed Python classes and remain independently testable.
    
- Existing Conservative, Moderate, Aggressive, Buzz, and Nightly behaviour can be reproduced after migration.
    
- `strategies.yaml` remains the source of truth until the UI persistence layer is implemented.
    
- Buzz can eventually expose modes such as:
...
- Changing strategy composition must not require modifying `Pipeline`.
    

### Technical Note

Do not implement this as part of US-006.

US-006 may temporarily keep hard-coded strategy composition.

`TechnicalStrategyFactory` should contain a `TODO(US-304)` marker until this refactor is completed.
# 技術債／Enabler Stories
...
3. 移除 repository 對缺失 `candidate_source` 的默認值。
    
4. 整理 AiReportParser 的資料驗證與錯誤處理。
