...
from src.a_domain.types.constants import REASON_NIGHTLY_SCREEN
    ...
    async def execute(self, universe: list[Stock]) -> SystemStats:
        ...
        for stock in universe:
            try:
                ...
                rsi, macd, ma = self._tech_calc.calculate_indicators(history)

                # Dry run policy
                ctx = AnalysisContext(
                    stock=stock,
                    source=CandidateSource.TECHNICAL_WATCHLIST,
                    trigger_reason=REASON_NIGHTLY_SCREEN,  # Using Constant
                    ohlcv_data=history,
